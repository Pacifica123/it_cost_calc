...
def test_yandex_market_http_job_spec_uses_non_playwright_cli_mode(tmp_path: Path) -> None:
    app = QtAppPresenter(
        repo_root=tmp_path,
        runtime_entities_path=tmp_path / "runtime.json",
    )
    presenter = CatalogStagingPresenter(app, staging_path=tmp_path / "staging.json")
    job = presenter.build_yandex_market_http_job(
        categories=["routers", "servers"],
    ...
    assert "yandex-market-http-live" in job.arguments
    assert "--browser-engine" not in job.arguments
    assert "--profile" not in job.arguments
    assert "routers,servers" in job.arguments
    assert "yandex_market_http_runs" in str(job.output_path)
