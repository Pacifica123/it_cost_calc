...
Допустимые категории совпадают с `DNS_CATEGORY_MAP`: `routers`, `prebuilt_pcs`, `servers`, `components`, `motherboards`, `cpus`, `gpus`, `rams`, `psus`, `ssds`, `hdds`, `cases`. Путь `file` обязан быть относительным, оставаться внутри каталога снимка и иметь HTML-расширение.
...
Управляемое создание такого снимка через видимый браузер описано в `dns_live_catalog_gui.md`.
