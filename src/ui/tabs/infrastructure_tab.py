"""UI tab preserved from legacy app; imports adjusted during stage-1 cleanup."""
...
class ITInfrastructureTab(tk.Frame):
    def __init__(self, parent, crud):
        ...
        self.create_article_button = tk.Button(
            self.inner_frame, text="Создать статью", command=self.create_article
        )
    ...
    def create_article(self):
        def save_article():
            ...
            tk.Button(
                new_table[2],
                text="Добавить",
                command=lambda table=new_table[1]: self.add_row(article_name, table),
            ).pack(side="left")
            tk.Button(
                new_table[2],
                text="Редактировать",
                command=lambda table=new_table[1]: self.edit_row(article_name, table),
            ).pack(side="left")
            tk.Button(
                new_table[2],
                text="Удалить",
            ...
            for column in table["columns"]:
                if column == "name":
                    ...
                    quantity_entry.insert(
                        0, self.crud.entities[entity_name][selected_index]["quantity"]
                    )
                    quantity_entry.pack()
                elif column == "monthly_cost":
                    tk.Label(popup, text="Ежемесячная стоимость").pack()
                    monthly_cost_entry = tk.Entry(popup)
                    monthly_cost_entry.insert(
                        0, self.crud.entities[entity_name][selected_index]["monthly_cost"]
                    )
                    monthly_cost_entry.pack()
                elif column == "one_time_cost":
                    tk.Label(popup, text="Разовая стоимость").pack()
                    one_time_cost_entry = tk.Entry(popup)
                    one_time_cost_entry.insert(
                        0, self.crud.entities[entity_name][selected_index]["one_time_cost"]
                    )
                    one_time_cost_entry.pack()

            tk.Button(popup, text="Сохранить", command=save_data).pack()
        else:
            logger.warning("Попытка редактирования без выбранной строки в ITInfrastructureTab")
